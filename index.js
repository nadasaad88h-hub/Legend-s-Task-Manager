...
// --- HELPERS ---
const hasAuth = (m) => m && ADMIN_ROLES.some(r => m.roles.cache.has(r));
...
async function nextRound(channel) {
  ...
  const embed = new EmbedBuilder().setTitle("🌍 Geography Quiz").setImage(currentRound.url).setColor(0xFFD700);
  await channel.send({ embeds: [embed], components: [row] }).catch(() => {});
}

// --- BOOT ---
client.once(Events.ClientReady, async () => {
  console.log("🛡️ Republic Online.");
  const rest = new REST({ version: "10" }).setToken(TOKEN);
  const cmds = [
    new SlashCommandBuilder().setName("promote").setDescription("Advance staff rank").addUserOption(o => o.setName("target").setRequired(true)).addStringOption(o => o.setName("ranks").setRequired(true).addChoices({name:'1 Rank', value:'1'}, {name:'2 Ranks', value:'2'})),
    new SlashCommandBuilder().setName("punish").setDescription("Executive punishment").addUserOption(o => o.setName("target").setRequired(true)).addStringOption(o => o.setName("type").setRequired(true).addChoices({name:'Timeout', value:'T'}, {name:'Suspension', value:'S'})).addStringOption(o => o.setName("reason").setRequired(true)),
    new SlashCommandBuilder().setName("daily").setDescription("Claim daily points"),
    new SlashCommandBuilder().setName("work").setDescription("Earn points (30m)"),
    new SlashCommandBuilder().setName("balance").setDescription("Check bank vault")
  ].map(c => c.toJSON());
...
client.on(Events.InteractionCreate, async (itx) => {
  ...
  if (itx.isButton()) {
    ...
    if (customId === "skip") {
      ...
      await itx.reply(`🚩 Skipped by <@${user.id}>. It was **${currentRound?.name}**.`);
  ...
  // Permissions Check
  const restricted = ["promote", "punish"];
  if (restricted.includes(commandName) && !hasAuth(member)) return itx.reply({ content: "Unauthorized.", ephemeral: true });

  if (commandName === "promote") {
    const target = options.getMember("target");
    if (!target) return itx.reply("Target not found.");
    const step = parseInt(options.getString("ranks"));
    const oldIdx = rankHierarchy.findIndex(r => target.roles.cache.has(r));
    const newIdx = Math.min(rankHierarchy.length - 1, (oldIdx === -1 ? 0 : oldIdx) + step);

    const toRemove = rankHierarchy.filter(r => target.roles.cache.has(r));
    if (toRemove.length) await target.roles.remove(toRemove).catch(() => {});
    await target.roles.add(rankHierarchy[newIdx]).catch(() => {});

    log(`Promotion: <@${target.id}> advanced to rank ${newIdx}`);
    return itx.reply(`✅ Success: <@${target.id}> promoted.`);
  }

  if (commandName === "punish") {
    const target = options.getMember("target");
    const type = options.getString("type");
    if (!target || !target.manageable) return itx.reply("Cannot touch this user.");

    if (type === "T") {
      await target.timeout(ms("1h"), options.getString("reason"));
    } else {
      const currentRoles = target.roles.cache.filter(r => r.id !== itx.guild.id).map(r => r.id);
      db.saveRoles(target.id, currentRoles);
      await target.roles.set([SUSPENDED_ROLE_ID]);
    }
    log(`Punishment: ${type} issued to <@${target.id}>`);
    return itx.reply("Order executed.");
  }

  if (commandName === "work") {
    const cd = getCD(user.id, "work", 1800000);
    if (cd) return itx.reply({ content: `Wait ${Math.ceil(cd/60000)}m.`, ephemeral: true });
    db.addPoints(user.id, 3);
    cooldowns.set(`${user.id}-work`, Date.now());
    return itx.reply("⚒️ Shift over. +3 Points.");
  }
  ...
  if (commandName === "daily") {
    ...
    return itx.reply("💰 Treasury issued +5 Points.");
  }
...
client.on(Events.MessageCreate, async (msg) => {
  ...
  if (msg.content.toLowerCase().trim() === currentRound.name.toLowerCase()) {
    currentRound = null; // Lock state
    await msg.react("✅").catch(() => {});
    db.addPoints(msg.author.id, 2);
    setTimeout(() => { msg.delete().catch(() => {}); nextRound(msg.channel); }, 1500);
  }
});
